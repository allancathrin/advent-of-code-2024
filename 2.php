...
function step2()
{
    ...
    foreach (getReports() as $report) {
        for ($i = 0; $i < count($report); $i++) {
            if (!empty($count[$iterator])) {
                break;
            }

            $cloneReport = $report;
            unset($cloneReport[$i]);
            $cloneReport = array_values($cloneReport);

            if (!isSafe($cloneReport)) {
                continue;
            }
    ...
    echo sprintf('Step 2 : %s', count($count));
}

function isSafe(array $report)
{
    $number = null;
    $order = null;
